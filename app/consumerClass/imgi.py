'''
Processor For Single Image Page Invoice
Functionality:
    - Define Class for Single Image Page Invoice
'''
...
logging.basicConfig(format = '%(asctime)s %(message)s',
                    filemode = 'w',
                    level=logging.INFO)
logger4.addHandler(logging.FileHandler('/app_dev/app/logging/splitting.log'))
...
logger4.info(f'----------------------------------------------------------------')
logger4.info(f'Module: IMGI | Single Page Image Invoice Processor Intitalized.')
logger4.info(f'----------------------------------------------------------------\n')
...
class IMGI:
    ...
    def process(self, partitionID, producerTopic, message, producer):
        ...
        '''
        Assumption will be that it is a single page image invoice.
        arguments:
        ...
        logger4.info(f'Module: IMGI, Parition {partitionID} | SPLITTING REQUEST ACCEPTED')
        logger4.info(f'Module: IMGI, Parition {partitionID} | Case ID is {caseId} | Parent Case ID is {parent_caseId} | Single Page PDF | Document Name is {imgname}.')
        ...
        logger4.info(f'Module: IMGI, Parition {partitionID} | {caseId} - Extension type is {extensionType}')

        try:

            if extensionType.lower() == 'tiff' or extensionType.lower() == 'tif':
                logger4.warning(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | INVOICE IS NOT A PDF - TIFF Detected. Converting!')
                s3 = boto3.client('s3')
                ...
                logger4.warning(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | INVOICE IS NOT A PDF - Image Detected. Converting!')
                s3 = boto3.client('s3')
                filename = '/app_dev/app/tempstorage/' + caseId + '.' + extensionType.lower()
                s3_path_to_delete = filename
                s3.download_file(bucket, path, filename)
                filename = '/app_dev/app/tempstorage/' + caseId + '.' + extensionType.lower()
                logger4.info(f'Image Path is {filename}')
            ...
            logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | File Downloaded From S3 Successfully.')

        except Exception as e:

            try:
            
                logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | File was not available on S3. Sleeping for 10 seconds and retrying for 2 times!')
                time.sleep(10)
                ...
                logger4.info(f'File Downloaded From S3 Successfully.')
        ...
        if len(pdf.pages) == 1:
            ...
            if parent_caseId == '':
                ...
                # Extract PO Numbers
                logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | Parent Case ID not specified. Assuming New Case. No Cache Located.')
                ...
                logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | Extracted Date and cache stored with path in S3 as {s3pathCache}.')
                ...
                logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | Parent Case ID specified. Looking In Cache Path {cachePath} - Using page number {get_s_page}.')
                ...
                if os.path.isfile(path):
                    ...
                    logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | CACHE FOUND IN ZCACHE! Using Preexisting Extract.')

                else:
                    logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | Cache Not Found In ZCACHE! Extracting.')
                    out, pages_out, key_val, tables = model.split_document(filename)
                    extracts[0] = out
                    po_numbers[0] = ext2apflow.split_pofinder(out, key_val)
                    to_save = [out, pages_out, key_val, tables]
            ...
            logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | SPLIT IS FALSE - Single Page Invoice.')
            ...
            try:
                ...
                logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | PO IS FOUND, SENDING FOR ERP IDENTIFICATION')

                # Write to Kafka
                try:
                    producer.send(producerTopic, json.dumps(out_message).encode('utf-8'), partition = partitionID)
                    producer.flush()
                    logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | Message Sent To Producer Topic.')
                except Exception as e:
                    logger4.error(e)
                    logger4.error(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | Producer Failed - Retrying')
                    time.sleep(5)
                    producer.send(producerTopic, json.dumps(out_message).encode('utf-8'), partition = partitionID)
                    producer.flush()
                ...
                if po_num != '':
                    ...
                    logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | PO Found - Sending to ERP Identification.')
                    ...
                    logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | PO EXTRACTION FAILED - Moving to direct extraction.')

                    # Write to Kafka
                    try:
                        producer.send(extractionIn, json.dumps(out_message).encode('utf-8'), partition = 0)
                        producer.flush()
                        logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | Message Sent To Extraction Topic.')
                    except Exception as e:
                        logger4.error(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | Producer Failed - Retrying')
                        time.sleep(5)
                        producer.send(extractionIn, json.dumps(out_message).encode('utf-8'), partition = 0)
            ...
            logger4.info(f'Module: IMGI, Parition {partitionID}, Case ID {caseId} | Completed Process For - Case ID {caseId} | Parent Case {parent_caseId} | Document Name {imgname}.')
            return caseId, 200
